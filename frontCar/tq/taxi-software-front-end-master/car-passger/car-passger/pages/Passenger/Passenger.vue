	...
	export default {
		data() {
			return {
				...
				timeid: null, //定时器
				pollingTime: 0, //轮询时间
				failedCount: 0, // 统计失败次数
				maxFailedCount: 20, // 最大重试次数
				retryInterval: 3000, // 重试间隔时间（3秒）
			...
			upDate() {
				uni.request({
					...
					success: (res) => {
						console.log(res);
						if (res.statusCode == 200) {
							if (res.data.data.orderplus.code != "0") {
								this.driverStatusInfo = '叮！司机正在赶来的路上~';
								clearTimeout(this.timerId);
								return;

							} else {
								console.log('暂无司机接单', res.data.data.orderplus.code);
